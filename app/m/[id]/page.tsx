...
async function getMovie(id: string) {
  return tmdbGet<MovieDetails>(`/movie/${id}`, {}, { revalidateSeconds: 60 * 60 * 6 });
}

async function getVideos(id: string) {
  return tmdbGet<Videos>(`/movie/${id}/videos`, {}, { revalidateSeconds: 60 * 60 * 12 });
}

async function getProviders(id: string) {
  return tmdbGet<WatchProviders>(`/movie/${id}/watch/providers`, {}, { revalidateSeconds: 60 * 60 * 24 });
}
...
export default async function MoviePage({
  ...
  const [movie, vids, providers, recData] = await Promise.all([
    getMovie(id),
    getVideos(id),
    getProviders(id),
    getRecommendations(seedId, "movie", { boosts: [] as any }),
  ]);

  const name = movie.title ?? "Untitled";
    ...
    <>
        ...
        <Container className="relative py-10 sm:py-12 md:py-14">
          <div className="flex flex-col gap-8 md:flex-row md:items-end">
            <div className="relative h-[240px] w-[168px] shrink-0 overflow-hidden rounded-[28px] bg-white ring-1 ring-slate-200/70 shadow-soft">
              {movie.poster_path ? (
                <Image src={tmdbImageUrl(movie.poster_path, "w500")!} alt={name} fill className="object-cover" priority />
              ) : null}
            </div>
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
                ...
                {runtime ? (
                  <span className="rounded-full bg-white px-3 py-1.5 ring-1 ring-slate-200">{runtime} min</span>
                ) : null}
      ...
      <Container className="py-10 sm:py-12 md:py-14">
        <MovieRecsClient
          ...
          seedType="movie"
          seedTitle={name}
          initialItems={recData.items as any}
          tunePack={toTunePackClient(recData.tunePack)}
        />

        <div className="mt-8 flex justify-end">
          <Link
            href={`/search?q=${encodeURIComponent(name)}&mode=auto`}
